...
$('.btn').eq(1).click(function(){
    $('.world').css({color:'rgba(19, 120, 30, 0.51)'})
});
$('.btn').eq(2).click(function(){
    $('.world').css({color:'rgba(45, 144, 21, 0.56)'})
})
...
(function(){
    ...
    function hoverDir(ev,obj){
        console.log($(obj).position().left,ev.pageX,$(obj).position().top,ev.pageY)
        var a = ev.pageX-$(obj).offset().left-obj.offsetWidth/2;
        var b = $(obj).offset().top+obj.offsetHeight/2-ev.pageY;
...
var gdz = oC2.getContext('2d');
var cx = oC2.width/2,
    cy = oC2.height/2;
function clock(){
    gdz.clearRect(0,0,oC2.width,oC2.height);
    var oDate = new Date();
    ...
    gdz.font = '20px 微软雅黑';
    gdz.textAlign = 'center';
    gdz.textBaseline = 'middle';
    gdz.fillText(str,cx,cy);
}
clock()
setInterval(clock,1);
...
function drawArc(r,s,e,color){
    color = color||'black';
    gdz.beginPath();
    gdz.arc(cx,cy,r,d2a(s-90),d2a(e-90),false);
    gdz.strokeStyle = color;
    gdz.lineWidth = 20;
    gdz.stroke();
}
